...
function decodeMatrix(matrix, options) {
    ...
    // ★ RS誤り訂正前に、EC情報とtotalCodewordsを計算
    ...
    // ★ originalCodewordsをtotalCodewordsに合わせてトリミング（readCodewordsが余剰ビットを返す場合に備える）
    ...
    // ★ RS訂正済みブロックからgetDataBlocksと逆の手順でインタリーブ再構築
    // getDataBlocksの読み取りパターンをそのまま逆再生する（blockPosトラッキング方式）
...
(function(root) {
    ...
    function dmNormalizeModuleCoord(nrow, ncol, row, col) {
        var r = row, c = col;
        if (r < 0) {
            r += nrow;
            c += 4 - ((nrow + 4) % 8);
    ...
    function dmMarkVisited(visited, nrow, ncol, coords, wrapped) {
        for (var i = 0; i < coords.length; i++) {
            var coord = wrapped ? dmNormalizeModuleCoord(nrow, ncol, coords[i][0], coords[i][1]) : coords[i];
            if (!coord) continue;
            var r = coord[0], c = coord[1];
            if (visited[r]) visited[r][c] = true;
        }
    }
    ...
    function dmReadCodewordsFromMapping(mapping, nrow, ncol, totalCodewords) {
        var codewords = [];
        var visited = [];
        for (var vr = 0; vr < nrow; vr++) visited[vr] = new Array(ncol).fill(false);
        var row = 4;
        var col = 0;
        do {
            if (row === nrow && col === 0) {
                codewords.push(dmReadCorner1(mapping, nrow, ncol));
                dmMarkVisited(visited, nrow, ncol, [[nrow - 1,0],[nrow - 1,1],[nrow - 1,2],[0,ncol - 2],[0,ncol - 1],[1,ncol - 1],[2,ncol - 1],[3,ncol - 1]], false);
            } else if (row === nrow - 2 && col === 0 && ncol % 4 !== 0) {
                codewords.push(dmReadCorner2(mapping, nrow, ncol));
                dmMarkVisited(visited, nrow, ncol, [[nrow - 3,0],[nrow - 2,0],[nrow - 1,0],[0,ncol - 4],[0,ncol - 3],[0,ncol - 2],[0,ncol - 1],[1,ncol - 1]], false);
            } else if (row === nrow - 2 && col === 0 && ncol % 8 === 4) {
                codewords.push(dmReadCorner3(mapping, nrow, ncol));
                dmMarkVisited(visited, nrow, ncol, [[nrow - 3,0],[nrow - 2,0],[nrow - 1,0],[0,ncol - 2],[0,ncol - 1],[1,ncol - 1],[2,ncol - 1],[3,ncol - 1]], false);
            } else if (row === nrow + 4 && col === 2 && ncol % 8 === 0) {
                codewords.push(dmReadCorner4(mapping, nrow, ncol));
                dmMarkVisited(visited, nrow, ncol, [[nrow - 1,0],[nrow - 1,ncol - 1],[0,ncol - 3],[0,ncol - 2],[0,ncol - 1],[1,ncol - 3],[1,ncol - 2],[1,ncol - 1]], false);
            }
            do {
                if (row < nrow && col >= 0 && !visited[row][col]) {
                    codewords.push(dmReadUtah(mapping, nrow, ncol, row, col));
                    dmMarkVisited(visited, nrow, ncol, dmUtahCoords(row, col), true);
                }
                row -= 2;
                col += 2;
    ...
    function getManagedCode32(result) {
        if (!result) return null;
        if (result.managementCode32 !== undefined && result.managementCode32 !== null) {
            return result.managementCode32 >>> 0;
        }
        if (result.managementCode !== undefined && result.managementCode !== null &&
            result.managementFlags16 !== undefined && result.managementFlags16 !== null) {
            return ((((result.managementCode & 0xFFFF) << 16) | (result.managementFlags16 & 0xFFFF)) >>> 0);
        }
        return null;
    }

    function acceptsDecodedDataMatrix(decoded, options) {
        if (!decoded || decoded.isRaw) return false;
        options = options || {};
        var text = decoded.text !== undefined ? decoded.text : decoded.data;
        if (options.requireNonEmptyText && !text) return false;
        var managementCode32 = getManagedCode32(decoded);
        if (options.requireManagementCode && managementCode32 === null) return false;
        if (options.expectedQrNo !== undefined && options.expectedQrNo !== null) {
            if (managementCode32 === null) return false;
            if (((managementCode32 >>> 30) & 0x3) !== (options.expectedQrNo & 0x3)) return false;
        }
        return true;
    }

    function rejectedDecodeAsRaw(decoded) {
        if (!decoded) return null;
        return {
            isRaw: true,
            finderScore: decoded.finderScore || 0,
            symbolSize: decoded.symbolSize,
            codewords: decoded.codewords,
            rejectedDecoded: true
        };
    }
    ...
    function decodeDataMatrixFromQuad(imageData, width, height, quad, options) {
        ...
        for (var pi = 0; pi < pads.length; pi++) {
            for (var i = 0; i < sizes.length; i++) {
                ...
                if (!decoded.isRaw) {
                    if (acceptsDecodedDataMatrix(decoded, options)) return decoded;
                    var rejected = rejectedDecodeAsRaw(decoded);
                    if (rejected && (!bestRaw || (rejected.finderScore || 0) > (bestRaw.finderScore || 0))) bestRaw = rejected;
                    continue;
                }
